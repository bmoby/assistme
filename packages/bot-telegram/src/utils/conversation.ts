/**
 * Conversation history for the admin bot.
 * Keeps recent messages so the orchestrator has context.
 */
...
const TTL_MS = 60 * 60 * 1000; // 1 hour (admin conversations are longer)
...
export function formatHistoryForPrompt(chatId: string): string {
  ...
  return history
    .map((m) => `${m.role === 'user' ? 'Magomed' : 'Copilote'}: ${m.text}`)
    .join('\n');
}
