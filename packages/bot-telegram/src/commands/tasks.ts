...
export function registerTaskCommands(bot: Bot): void {
  ...
  // /done - Complete current/specified task
  bot.command('done', async (ctx: Context) => {
    ...
    if (arg) {
      // Complete specific task by number in the list
      ...
      // Complete the next/current task
      const task = await getNextTask();
      if (!task) {
  ...
  bot.command('skip', async (ctx: Context) => {
    ...
    // Get current top task and move it down by changing priority
    const tasks = await getActiveTasks();
    if (tasks.length === 0) {
    ...
    // Show the next task after the skipped one
